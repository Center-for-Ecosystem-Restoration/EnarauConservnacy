# Human-pressure / road / hydrology / terrain derived layers on the 30m master grid (plan
# Sec.5.3-5.6). Each function returns one aligned, named SpatRaster layer.

#' Settlement-pressure raster: 0.70*heatmap_scaled + 0.30*built_fraction (plan Sec.5.3). The
#' heatmap is percentile-scaled (not raw min-max) for consistency with every other continuous
#' input in this pipeline; built_fraction is assumed already in [0, 1] (a class_fraction_stack()
#' grouping) and used as-is.
#'
#' NA FILL (found 2026-07-29, via a Circuitscape pairwise run reporting most focal-node pairs as
#' graph-disconnected): settlement_heatmap.tif's own valid-data footprint -- however it was
#' clipped/masked in QGIS -- does NOT match the land-cover-based AOI mask every other layer in
#' this pipeline shares; it's missing an *additional* ~16% of the grid (29,448 cells) that
#' land-cover, slope, and condition all consider valid, scattered through the interior rather than
#' confined to one edge. Left unfilled, this NA propagates into settlement_pressure ->
#' resistance_B/resistance_C, and Circuitscape treats NA as an absolute graph barrier (not merely
#' high resistance), which silently splits the landscape into disconnected components -- 4 of the
...
#' point by construction, so filling its own NA with 0 here is conservative and reasonable; the
#' TRUE AOI boundary is still enforced downstream by every other resistance-formula input, so this
#' fill can never incorrectly extend the analysis area.
...
#' the max across classes at every cell (the closest/highest-scoring road wins) (plan Sec.5.4).
...
#' Distance (m), per master-grid cell, to the nearest feature in a line/point vector layer (plan
#' Sec.5.5's river_proximity_30m.tif).
...
#' Binary riparian buffer: 1 where `river_distance` <= RIPARIAN_BUFFER_M, else 0 (plan
#' Sec.3.1.D/5.5). NOT the resistance-model riparian_factor scenario values (Sec.7.3) -- those are
#' built downstream from this buffer once Resistance Model C is assembled.
...
#' Percentile-scaled slope (plan Sec.5.6).
...
#' Terrain Ruggedness Index from elevation, aligned to the master grid (plan Sec.5.6's optional
#' "terrain ruggedness index or local relief").
